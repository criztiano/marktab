...
- **Network:** the only network layer is the optional "Try next" feed (`entrypoints/newtab/feed.ts` + `TryNext.tsx`). No host is hardcoded and none is requested at install — the feed is dormant until the user sets a base URL in Settings; saving triggers a per-host `optional_host_permissions` grant (`optional_host_permissions: ['https://*/*','http://*/*']` in `wxt.config.ts`). Base URL + token live in `browser.storage.local` (`storage` permission); `loadConfig` migrates the legacy `eden*` keys. Keep all other surfaces request-free.
